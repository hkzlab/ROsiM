...
void fill_sipo_buffer(uint32_t address, uint16_t data) {
    ...
    // Address
    for(idx = 18, bit_cnt = 0; idx >= 0; idx--, bit_cnt++) { // We have A0-A18, thus we start with idx 18
    ...
    // Data
    for(idx = 15, bit_cnt = 0; idx >= 0; idx--, bit_cnt++) { // We have D0-A15, thus we start with idx 15
