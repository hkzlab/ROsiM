...
void data_to_sipo_buffer(uint16_t data) {
    ...
    for(int8_t idx = 15, bit_cnt = 0; idx >= 0; idx--, bit_cnt++) { // We have D0-A15, thus we start with idx 15
...
void address_to_sipo_buffer(uint32_t address) {
    ...
    for(int8_t idx = 18, bit_cnt = 0; idx >= 0; idx--, bit_cnt++) { // We have A0-A18, thus we start with idx 18
