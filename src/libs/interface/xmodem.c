...
static void xmodem_upload_packet(void);
...
uint8_t xmodem_xfer(XMODEM_Dump_Type dtype) {
    ...
    while(1) {
        if(xmodem_recv_pkt()) {
            switch(packet_buf[0]) {
                case SOH:
                    if(!xmodem_check_packet()) uart_putchar(NACK);
                    else {
                        last_pkt = packet_buf[1];
                        xmodem_upload_packet();
                        uart_putchar(ACK);
                    }
                    break;
...
static uint8_t xmodem_check_packet(void) {
    ...
    if(crc != calc_crc) return 0;
    if(packet_buf[1] != expected_packet) return 0;
